...
<head>
    ...
    <style>
        ...
        .legal-text { font-size: 10px; line-height: 1.6; opacity: 0.7; }
    </style>
</head>
<body class="h-screen flex flex-col">
    ...
    <main id="app-ui" class="hidden flex-1 overflow-y-auto pb-32">
        ...
        <div id="p-invest" class="page p-6">
            <h2 class="text-center font-black italic mb-8 uppercase text-blue-500 text-xl tracking-widest underline decoration-4 underline-offset-8">Trading Fleet</h2>
        ...
        <div id="p-more" class="page p-6 space-y-6 pb-12">
            <div class="glass p-8 rounded-[3rem] text-center">
                <p class="text-[8px] font-bold text-gray-500 mb-4 uppercase">Your Referral Network</p>
            ...
            <div class="glass p-8 rounded-[3rem]">
                ...
                <p class="legal-text">MintCrest Gold Enterprise is a premium custodial service specializing in digital asset management. We operate on a 30-day fixed-term trading cycle to ensure liquidity and stable returns for our global partners.</p>
            </div>

            <div class="glass p-8 rounded-[3rem]">
                <h3 class="text-blue-500 font-black text-xs uppercase mb-4 italic">Privacy & Security</h3>
                <p class="legal-text">1. All user data is encrypted via 256-bit AES protocols.<br>2. We do not share account details with third-party aggregators.<br>3. Withdrawals are processed after manual identity verification.</p>
            </div>

            <div class="glass p-8 rounded-[3rem]">
                <h3 class="text-blue-500 font-black text-xs uppercase mb-6 italic text-center">Contact Support</h3>
                <textarea id="support-msg" placeholder="Describe your issue..." class="w-full bg-white/5 p-5 rounded-2xl text-[10px] border border-white/10 outline-none mb-4 h-32"></textarea>
        ...
        <div id="p-withdraw" class="page p-6">
            <div class="glass p-10 rounded-[3.5rem] border-t-8 border-red-600">
                ...
                <input type="text" id="wd-acc" placeholder="Account Name & Number" class="w-full bg-white/5 p-5 rounded-2xl mb-8 text-center text-[9px] font-bold">
    ...
    <div id="admin-panel" class="fixed inset-0 bg-[#000103] z-[5000] p-6 hidden overflow-y-auto">
        ...
        <div class="glass p-8 rounded-[3rem] border border-white/5 mt-10">
            <h4 class="text-[9px] font-black text-center mb-6 opacity-30 uppercase">Manual Injection</h4>
            <input type="text" id="adm-user" placeholder="User Name" class="w-full bg-white/5 p-5 rounded-2xl mb-2 text-center font-bold outline-none border border-white/5">
            <input type="number" id="adm-val" placeholder="Amount (PKR)" class="w-full bg-white/5 p-5 rounded-2xl mb-6 text-center font-bold outline-none border border-white/5">
    ...
    <script>
        ...
        const plans = [
            { n: "Starter-I", p: 500, r: 4 }, { n: "Bronze-I", p: 2000, r: 4.5 }, { n: "Silver-I", p: 5000, r: 5 }, { n: "Gold-I", p: 15000, r: 6 }, { n: "Platinum-I", p: 30000, r: 7 }, { n: "Diamond-I", p: 60000, r: 8 }, { n: "Royal Master", p: 100000, r: 10 }
        ];
        ...
        async function sendSupport() {
            ...
            await db.collection("requests").add({ user: user.name, amount: 0, type: "Support Ticket", status: "pending", message: msg, time: Date.now() });
            document.getElementById('support-msg').value = '';
            alert("Ticket Submitted! Admin will review it.");
        }
        ...
        function updateCountdown() {
            if (user && user.activeTier > 0) {
                ...
                if (diff > 0) {
                    ...
                    document.getElementById('countdown-display').innerText = `YIELD REQ IN: ${h}h ${m}m ${s}s`;
                } else { document.getElementById('countdown-display').innerText = "YIELD REQ SENT"; }
            } else { document.getElementById('countdown-display').innerText = "VAULT OFFLINE"; }
        }
        ...
        function renderPlans() { 
            ...
            plans.forEach(p => { 
                list.innerHTML += `<div onclick="buy(${p.p}, ${p.r}, '${p.n}')" class="glass p-6 rounded-[2.5rem] flex justify-between items-center active:scale-95 mb-2 cursor-pointer transition-all border border-white/5"><div><h4 class="font-black text-[11px] uppercase text-blue-500">${p.n}</h4><p class="text-[9px] text-green-400 font-bold uppercase">${p.r}% Daily / 30 Days</p></div><p class="font-black text-sm">₨ ${p.p.toLocaleString()}</p></div>`; 
            }); 
        }
        ...
        async function syncAdmin() {
            db.collection("requests").where("status", "==", "pending").onSnapshot(snap => {
                const reqList = document.getElementById('adm-sec-requests');
                const supList = document.getElementById('adm-sec-support');
                reqList.innerHTML = ''; supList.innerHTML = '';
                snap.forEach(doc => {
                    const d = doc.data();
                    const ui = `<div class="glass p-5 rounded-2xl flex justify-between items-center mb-2 text-[9px] font-black uppercase"><div>${d.user}<br>${d.type === 'Support Ticket' ? d.message : '₨ '+d.amount+' ('+d.type+')'}</div><div class="flex gap-2"><button onclick="handleReq('${doc.id}', '${d.user}', ${d.amount}, 'approved', '${d.type}')" class="bg-green-600 px-5 py-2 rounded-xl">YES</button><button onclick="handleReq('${doc.id}', '${d.user}', ${d.amount}, 'rejected', '${d.type}')" class="bg-red-600 px-5 py-2 rounded-xl">NO</button></div></div>`;
                    if(d.type === 'Support Ticket') supList.innerHTML += ui; else reqList.innerHTML += ui;
                });
            });
            db.collection("users").onSnapshot(snap => {
                const list = document.getElementById('adm-sec-users'); list.innerHTML = '';
                snap.forEach(doc => { const u = doc.data(); list.innerHTML += `<div class="glass p-5 rounded-2xl flex justify-between items-center mb-2 text-[9px] font-black uppercase"><div>${u.name}</div><div>Bal: ${u.balance}<br>Inv: ${u.activeTier || 0}</div><button onclick="deleteUser('${u.name}')" class="text-red-500">×</button></div>`; });
            });
        }
