...
<head>
    ...
    <title>MintCrest Gold | v28.0 Fixed</title>
    <style>
...
<body>
    ...
    <main id="app-ui" class="hidden p-4 space-y-6">
        <div id="p-home" class="page active-page space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <a href="https://chat.whatsapp.com/YOUR_LINK" class="glass p-6 rounded-[2.5rem] flex flex-col items-center gap-2 text-green-500">
                    <i class="fa-brands fa-whatsapp text-3xl"></i><span class="text-[9px] font-black uppercase">Community</span>
                </a>
                <button onclick="changePage('about')" class="glass p-6 rounded-[2.5rem] flex flex-col items-center gap-2 text-blue-400">
                    <i class="fa-solid fa-shield-halved text-3xl"></i><span class="text-[9px] font-black uppercase">About Us</span>
                </button>
            </div>
        </div>

        <div id="p-nodes" class="page space-y-4 pb-24"><div id="plans-grid" class="grid grid-cols-1 gap-4"></div></div>
        ...
        <div id="p-wallet" class="page"><div class="glass p-8 rounded-[3.5rem] space-y-6 text-center">
            ...
            <div class="space-y-3">
                <div class="p-4 bg-white/5 border border-white/5 rounded-2xl">
                    <p class="text-[8px] uppercase opacity-40 mb-1">JazzCash</p>
                    <h4 id="j-num-disp" class="text-xl font-black text-white italic">Loading...</h4>
                </div>
                <div class="p-4 bg-white/5 border border-white/5 rounded-2xl">
                    <p class="text-[8px] uppercase opacity-40 mb-1">EasyPaisa</p>
            ...
            <div class="text-left"><label class="text-[10px] font-black uppercase ml-2 opacity-50">Upload Proof:</label>
            <input id="d-proof-file" type="file" accept="image/*" class="mt-2 text-[10px] file:bg-blue-600 file:border-none file:text-white file:rounded-lg file:px-2 cursor-pointer"></div>
        ...
        <div id="p-about" class="page space-y-4"><div class="glass p-8 rounded-[3.5rem] italic text-[12px] opacity-70">
            ...
            <p>Email: webhub262@gmail.com</p><p>Powered by Prime Solutions Digital.</p>
        </div></div>
    </main>

    <div id="admin-ui" class="fixed inset-0 bg-[#010409] z-[100000] hidden overflow-y-auto p-6 pb-40">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-2xl font-black text-blue-500 italic uppercase">Admin Hub</h2>
        ...
        <div class="admin-card">
            <h4 class="text-[10px] font-black uppercase text-blue-400 mb-4">Payment Setup</h4>
            <input id="adm-jnum" placeholder="New JazzCash Num">
            <input id="adm-enum" placeholder="New EasyPaisa Num">
            <button onclick="savePaymentSettings()" class="w-full p-3 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase">Save Numbers</button>
        </div>

        <div class="admin-card">
            <h4 class="text-[10px] font-black uppercase text-red-400 mb-4">Maintenance</h4>
            <div class="flex gap-2">
                <button onclick="toggleMaint(true)" class="flex-1 bg-red-600 p-3 rounded-xl text-[9px] font-black uppercase">ON</button>
                <button onclick="toggleMaint(false)" class="flex-1 bg-green-600 p-3 rounded-xl text-[9px] font-black uppercase">OFF</button>
            </div>
        </div>

        <h4 class="text-[12px] font-black uppercase text-green-500 italic mb-4">Requests Proofs</h4>
    ...
    <nav id="bottom-nav" class="hidden fixed bottom-0 w-full glass border-t border-[var(--border)] p-6 flex justify-around items-center rounded-t-[3.5rem] z-[4000]">
        ...
        <button onclick="logout()"><i class="fa-solid fa-right-from-bracket text-red-500"></i></button>
    </nav>
    ...
    <section id="auth-ui" class="fixed inset-0 z-[20000] bg-[var(--bg)] flex flex-col items-center justify-center p-10 text-center">
        ...
        <button onclick="login()" class="w-full max-w-[320px] p-6 btn-prime uppercase text-[12px] font-black tracking-widest">Login</button>
    </section>
    ...
    <script>
        ...
        function login() {
            ...
            if(n.length < 3) return alert("Enter name! 😘");
        ...
        async function enterApp(n) {
            ...
            db.collection("settings").doc("global").onSnapshot(d => {
                if(!d.exists) { db.collection("settings").doc("global").set({ maintenance: false, jazzNum: 'Set Now', easyNum: 'Set Now' }); }
                settings = d.data() || {};
                document.getElementById('maint-screen').style.display = settings.maintenance ? 'flex' : 'none';
                document.getElementById('j-num-disp').innerText = settings.jazzNum || '03xx...';
                document.getElementById('e-num-disp').innerText = settings.easyNum || '03xx...';
            });
        ...
        async function savePaymentSettings() {
            ...
            const upData = {};
            if(j) upData.jazzNum = j;
            if(e) upData.easyNum = e;
            await db.collection("settings").doc("global").set(upData, { merge: true });
        ...
        async function sendDeposit() {
            ...
            if(!a || a < 200 || !t || !file) return alert("Proof upload karein aur details bharein! 😘");
            ...
            db.collection("requests").where("status", "==", "pending").onSnapshot(s => {
                ...
                s.forEach(doc => {
                    ...
                    list.innerHTML += `<div class="glass p-5 rounded-[2rem] border-l-4 border-yellow-500 mb-2">
                        ...
                        <img src="${r.proof}" class="w-full rounded-xl my-3 border border-white/10">
                        <button onclick="approveReq('${doc.id}', '${r.user}', ${r.amount})" class="w-full bg-green-600 p-3 rounded-xl text-[10px] font-black uppercase">Approve</button>
                    </div>`;
                });
            });
        }
        ...
        function syncPlans() {
            db.collection("plans").orderBy("price", "asc").onSnapshot(s => {
                ...
                s.forEach(doc => { const p = doc.data(); g.innerHTML += `<div class="glass p-7 rounded-[2.8rem] flex justify-between items-center"><div><h4 class="text-2xl font-black italic">₨ ${p.price}</h4><p class="text-[8px] font-bold text-blue-400 uppercase">${p.name}</p></div><button onclick="buyPlan('${doc.id}', ${p.price})" class="btn-prime px-8 py-4 text-[11px] font-black uppercase">Buy</button></div>`; });
            });
        }
