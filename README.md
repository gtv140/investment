...
<head>
    ...
    <title>MintCrest Gold | v24.0 Fix</title>
    <style>
...
<body>
    ...
    <div id="ban-screen" class="full-overlay">
        ...
        <p class="opacity-50 text-[12px] italic mt-2">Access revoked by Admin. 😘</p>
    </div>

    <div id="maint-screen" class="full-overlay bg-slate-950">
        <i class="fa-solid fa-screwdriver-wrench text-blue-500 text-6xl mb-4 animate-bounce"></i>
        <h1 class="text-2xl font-black uppercase text-white">Maintenance Mode</h1>
    ...
    <header id="main-header" class="hidden sticky top-0 z-[5000] bg-[var(--bg)]/90 backdrop-blur-xl border-b border-[var(--border)] p-4 flex justify-between items-center">
        ...
        <div class="flex items-center gap-3">
            <div id="top-bal" class="text-[10px] font-black bg-blue-600/10 text-blue-500 px-4 py-2 rounded-full border border-blue-500/20 italic tracking-tighter">₨ 0</div>
    ...
    <main id="app-ui" class="hidden p-4 space-y-6">
        <div id="p-home" class="page active-page space-y-6">
            <div id="global-notice-bar" class="hidden glass p-3 rounded-2xl text-[10px] font-black uppercase text-blue-400 border-blue-500/20 italic"><i class="fa-solid fa-circle-check mr-2"></i> <span id="ntc-txt"></span></div>
            
            <div class="p-8 rounded-[3rem] bg-gradient-to-br from-blue-700 to-indigo-950 text-white shadow-2xl relative overflow-hidden">
                <p class="text-[10px] font-black uppercase opacity-60">Available Balance</p>
            ...
            <div class="glass p-5 rounded-[2.5rem] flex gap-2">
                <input id="p-code-in" placeholder="ENTER PROMO CODE" class="m-0 text-[10px] font-black uppercase border-blue-500/10">
        ...
        <div id="p-history" class="page space-y-4 pb-24 text-center"><h3 class="font-black uppercase text-blue-500 italic">History Logs</h3><div id="user-logs-list" class="space-y-3"></div></div>

        <div id="p-wallet" class="page"><div class="glass p-8 rounded-[3.5rem] space-y-6 text-center">
            <h3 class="font-black uppercase text-blue-500">Deposit Hub</h3>
            ...
            <div class="p-3 bg-blue-600/5 rounded-xl border border-blue-500/10"><p class="text-[9px] uppercase opacity-40">Payment Number</p><h4 id="pay-num" class="text-lg font-black text-blue-400">---</h4></div>
            <input id="d-amt" type="number" placeholder="Amount (Min 200)"><input id="d-tid" placeholder="Transaction ID"><button onclick="sendDeposit()" class="w-full p-4 btn-prime text-[11px] font-black uppercase italic">Verify Payment</button>
        </div></div>

        <div id="p-withdraw" class="page"><div class="glass p-8 rounded-[3.5rem] space-y-6 text-center">
            <h3 class="font-black uppercase text-blue-500">Withdraw Funds</h3>
            <input id="w-amt" type="number" placeholder="Amount (Min 100)"><input id="w-acc" placeholder="Account Number"><button onclick="sendWithdraw()" class="w-full p-4 btn-prime text-[11px] font-black uppercase italic">Submit Request</button>
        </div></div>
    </main>

    <div id="admin-ui" class="fixed inset-0 bg-[#010409] z-[100000] hidden overflow-y-auto p-6 pb-32">
        <div class="flex justify-between items-center mb-6">
            <div><h2 class="text-xl font-black text-blue-500 uppercase italic">Goddess Admin</h2><p class="text-[8px] font-black opacity-40 uppercase">Users Online: <span id="adm-count" class="text-green-500">0</span></p></div>
        ...
        <div class="admin-section">
            <h4 class="text-[10px] font-black uppercase text-red-500 mb-4">Master Power</h4>
            <div class="flex gap-2">
                <button onclick="toggleMaint(true)" class="flex-1 bg-red-600/10 border border-red-600/20 p-3 rounded-xl text-[9px] font-black uppercase">Maintenance ON</button>
                <button onclick="toggleMaint(false)" class="flex-1 bg-green-600/10 border border-green-600/20 p-3 rounded-xl text-[9px] font-black uppercase">Maintenance OFF</button>
            </div>
            <input id="adm-notice" placeholder="Global Notice Message" class="mt-4 text-[11px]">
            <button onclick="saveNotice()" class="w-full p-3 bg-white/5 border border-white/20 rounded-xl text-[9px] font-black uppercase">Set Notice</button>
        </div>

        <div class="admin-section">
            <h4 class="text-[10px] font-black uppercase text-blue-400 mb-4">User Management</h4>
        ...
        <div class="admin-section">
            <h4 class="text-[10px] font-black uppercase text-purple-400 mb-4">Plan Creator</h4>
            <input id="pl-name" placeholder="Plan Name"><div class="flex gap-2"><input id="pl-price" type="number" placeholder="Price"><input id="pl-profit" type="number" placeholder="Daily %"></div>
            <button onclick="createPlan()" class="w-full p-3 bg-purple-600/20 text-purple-400 rounded-xl text-[9px] font-black uppercase">Create Node</button>
        </div>

        <div class="admin-section">
            <h4 class="text-[10px] font-black uppercase text-yellow-500 mb-4">Payments & Promo</h4>
            ...
            <button onclick="savePaymentSettings()" class="w-full p-3 btn-prime text-[9px] font-black mb-6">Save Details</button>
            <input id="adm-pcode" placeholder="Promo Code"><input id="adm-pval" type="number" placeholder="Value">
            <button onclick="addPromo()" class="w-full p-3 bg-yellow-600/20 text-yellow-500 rounded-xl text-[9px] font-black uppercase">Add Promo</button>
        </div>

        <h4 class="text-[11px] font-black uppercase text-green-500 mb-4 px-2">Pending Queue</h4>
    ...
    <section id="auth-ui" class="fixed inset-0 z-[20000] bg-[var(--bg)] flex flex-col items-center justify-center p-8">
        ...
        <input id="l-name" placeholder="YOUR USERNAME" class="max-w-[320px] text-center font-black uppercase italic border-blue-500/20">
        <button onclick="login()" class="w-full max-w-[320px] p-5 btn-prime uppercase text-[12px] font-black tracking-widest active:scale-95 transition-all">Enter Portal</button>
    </section>
    ...
    <nav id="bottom-nav" class="hidden fixed bottom-0 w-full glass border-t border-[var(--border)] p-5 flex justify-around items-center rounded-t-[3.5rem] z-[4000]">
        <button onclick="changePage('home')" class="flex flex-col items-center gap-1"><i class="fa-solid fa-home"></i><span class="text-[7px] font-black uppercase">Home</span></button>
        <button onclick="changePage('wallet')" class="flex flex-col items-center gap-1"><i class="fa-solid fa-wallet text-blue-500"></i><span class="text-[7px] font-black uppercase">Deposit</span></button>
        <button onclick="changePage('history')" class="flex flex-col items-center gap-1"><i class="fa-solid fa-list-ul"></i><span class="text-[7px] font-black uppercase">Logs</span></button>
    </nav>
    ...
    <script>
        ...
        function login() {
            ...
            if(n.length < 3) return alert("Invalid Name sweetie! 😘");
        ...
        async function enterApp(n) {
            ...
            db.collection("settings").doc("global").onSnapshot(d => {
                ...
                if(settings.maintenance === true) mScreen.style.display = 'flex';
                else mScreen.style.display = 'none';
            ...
            db.collection("users").doc(n).onSnapshot(doc => {
                ...
                user = doc.data();
                const bScreen = document.getElementById('ban-screen');
                if(user.status === 'banned') bScreen.style.display = 'flex';
                else bScreen.style.display = 'none';
        ...
        async function createPlan() {
            ...
            await db.collection("plans").add({ name: n, price: p, profit: pr }); alert("Success! 😘");
        }
            ...
            db.collection("users").onSnapshot(s => {
                ...
                s.forEach(doc => {
                    ...
                    l.innerHTML += `<div class="u-row flex justify-between items-center p-3 border-b border-white/5 uppercase text-[9px] font-bold" data-name="${u.name}">
                        ...
                        <div class="flex gap-2">
                            <button onclick="db.collection('users').doc('${u.name}').update({status: '${u.status==='active'?'banned':'active'}'})" class="${u.status==='active'?'text-red-500':'text-green-500'}">BAN/UNBAN</button>
                            <button onclick="db.collection('users').doc('${u.name}').update({balance: parseInt(prompt('New Balance:'))})" class="text-blue-500"><i class="fa-solid fa-edit"></i></button>
                        </div>
                    </div>`;
                });
            });
            db.collection("requests").where("status", "==", "pending").onSnapshot(s => {
                ...
                s.forEach(doc => {
                    ...
                    list.innerHTML += `<div class="glass p-4 rounded-3xl border-l-4 border-yellow-500 mb-2">
                        ...
                        <p class="text-[8px] opacity-40">${r.tid || r.acc}</p>
        ...
        async function applyPromo() {
            ...
            const data = d.data();
            if(data.usedBy.includes(user.name)) return alert("Already Used! 😘");
            ...
            alert(`Sweetie, ₨ ${data.value} Added! 😘`);
        }
        ...
        function syncPlans() {
            db.collection("plans").orderBy("price", "asc").onSnapshot(s => {
                ...
                s.forEach(doc => { const p = doc.data(); g.innerHTML += `<div class="glass p-6 rounded-[2.8rem] flex justify-between items-center border-l-4 border-blue-500 shadow-xl"><div><p class="text-[9px] font-black opacity-40 uppercase">${p.name}</p><h4 class="text-2xl font-black italic">₨ ${p.price}</h4><p class="text-[8px] font-bold text-blue-400">${p.profit}% Profit</p></div><button onclick="buyPlan('${doc.id}', ${p.price}, '${p.name}')" class="btn-prime px-7 py-4 text-[11px] font-black uppercase tracking-widest">Buy</button></div>`; });
            });
        }
        function syncLogs() {
            db.collection("logs").where("user", "==", user.name).orderBy("time", "desc").limit(10).onSnapshot(s => {
                ...
                s.forEach(doc => { const d = doc.data(); l.innerHTML += `<div class="glass p-4 rounded-2xl flex justify-between items-center border-l-2 ${d.status==='approved'?'border-green-500':'border-yellow-500'}"><p class="text-[10px] font-black uppercase italic">${d.msg}</p><span class="text-[8px] font-black uppercase ${d.status==='approved'?'text-green-500':'text-yellow-500'}">${d.status}</span></div>`; });
            });
        }
