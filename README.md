...
<body>
    ...
    <header class="p-4 flex justify-between items-center sticky top-0 bg-[var(--bg)]/80 backdrop-blur-md border-b border-[var(--border)] z-[5000]">
        ...
        <div class="flex items-center gap-3">
            ...
            <button onclick="logout()" class="w-8 h-8 glass rounded-full flex items-center justify-center text-xs">🚪</button>
        </div>
    </header>
    ...
    <main id="app-ui" class="hidden p-4 space-y-6">
        ...
        <div id="p-withdraw" class="page space-y-6">
            <h3 class="text-[10px] font-black uppercase opacity-40 text-center italic tracking-widest">Withdrawal (Tax: 10%)</h3>
            <div class="glass p-6 rounded-[2.5rem] space-y-4">
                <select id="w-method"><option value="">Select Method</option><option value="JazzCash">JazzCash</option><option value="EasyPaisa">EasyPaisa</option></select>
    ...
    <div id="admin-ui" class="fixed inset-0 bg-[#010409] z-[100000] hidden overflow-y-auto p-4 md:p-8">
        <div class="flex justify-between items-center mb-8 sticky top-0 bg-[#010409]/90 backdrop-blur-md py-4 z-10">
            <h2 class="text-xl font-black text-blue-500 uppercase italic">Administration</h2>
    ...
    <script>
        ...
        // --- PLANS (ADMIN APPROVED) ---
        const NORMAL_PLANS = Array.from({length: 10}, (_, i) => ({ name: `Node Standard v${i+1}`, price: 500 + (i*500), daily: 50 + (i*50) }));
        const SPECIAL_PLANS = [{ name: "VIP Elite Node", price: 10000, daily: 1500 }];
        ...
        function renderPlans(cat) {
            ...
            data.forEach(p => {
                grid.innerHTML += `<div class="glass p-6 rounded-[2.5rem] border-l-4 border-blue-500 flex justify-between items-center">
                    <div><h4 class="font-black text-xs uppercase">${p.name}</h4><p class="text-[10px] text-blue-500 font-bold mt-1">Price: ₨ ${p.price}</p></div>
                    <button onclick="buyPlan('${p.name}', ${p.price}, ${p.daily})" class="btn-prime px-6 py-3 text-[8px]">BUY NODE</button>
                </div>`;
            });
        }
        ...
        async function buyPlan(pName, price, daily) {
            ...
            if(confirm(`Request activation for ₨ ${price}?`)) {
        ...
        async function submitDep() {
            ...
            if(!a || a < 200 || !tid || !f) return alert("Fill details correctly (Min ₨ 200).");
        ...
        async function submitWith() {
            ...
            if(!a || a < 100 || !m || !ac || !nm) return alert("Fill all details (Min ₨ 100).");
        ...
        function syncAdm() {
            db.collection("requests").where("status", "==", "pending").onSnapshot(s => {
                ...
                s.forEach(doc => {
                    const r = doc.data();
                    let info = r.type === 'plan_activation' ? `Plan: ${r.plan} (₨ ${r.amount})` : r.type === 'withdrawal' ? `Pay: ₨ ${r.receive} (Tax: ₨ ${r.tax})` : `Dep: ₨ ${r.amount}`;
                    list.innerHTML += `<div class="glass p-6 rounded-3xl border-l-4 border-blue-500">
                        <div class="flex gap-4 items-center">
                            ...
                            <div class="flex-1 text-[10px] font-bold">
                                <p>${r.user.toUpperCase()} | ${r.type.replace('_',' ')}</p>
                                <p class="text-blue-400 mt-1 font-black">${info}</p>
                            </div>
        ...
        async function approve(id, u, type, amt, daily) {
            ...
            else if(type === 'plan_activation') await ref.update({ balance: firebase.firestore.FieldValue.increment(-amt), activePlan: true, dailyYield: daily });
