...
<body class="h-screen flex flex-col vibrant-bg">
    ...
    <script>
        ...
        async function submitWithdraw() {
            ...
            const accName = document.getElementById('wd-acc-name').value;
            const accNum = document.getElementById('wd-acc-num').value;
            ...
            await db.collection("requests").add({ 
                user: user.name, 
                amount: parseInt(a), 
                acc: detailStr, // Detailed string for admin
                type: "withdraw", 
                status: "pending", 
                time: Date.now() 
            });
        ...
        async function syncAdmin() {
            db.collection("requests").where("status", "==", "pending").onSnapshot(snap => {
                ...
                snap.forEach(doc => { 
                    const d = doc.data(); 
                    const pBtn = d.proof ? `<button onclick="window.open().document.write('<img src=\\'${d.proof}\\' style=\\'width:100%\\' />')" class="bg-blue-100 text-blue-600 p-2 rounded text-[8px] font-black">📸 PROOF</button>` : '';
                    
                    // Detailed withdrawal method display for admin
                    ...
                    list.innerHTML += `
                        <div class="glass-light p-5 rounded-2xl mb-4 border border-slate-100">
                            <div class="flex justify-between items-start text-[9px] font-black uppercase">
                                <div>
                                    <span class="text-blue-600">${d.user}</span><br>
                                    <span class="text-slate-800 text-sm">Rs ${d.amount.toLocaleString()}</span><br>
                                    <span class="opacity-50">${d.type}</span>
                                </div>
                                <div class="flex gap-2">
                                    ${pBtn}
                                    <button onclick="handle('${doc.id}','${d.user}',${d.amount},'approved','${d.type}')" class="bg-green-600 text-white px-3 py-2 rounded-lg">YES</button>
                                    <button onclick="handle('${doc.id}','${d.user}',${d.amount},'rejected','${d.type}')" class="bg-rose-600 text-white px-3 py-2 rounded-lg">NO</button>
                                </div>
                            </div>
                            ${withdrawDetails}
                        </div>`;
            ...
            db.collection("users").onSnapshot(snap => { const list = document.getElementById('adm-sec-users'); list.innerHTML = ''; snap.forEach(doc => { const u = doc.data(); list.innerHTML += `<div class="glass-light p-4 rounded-xl flex justify-between items-center text-[8px] font-black uppercase mb-2"><div>${u.name}</div><div>Bal: ${u.balance}</div><button onclick="deleteUser('${u.name}')" class="text-rose-600"><i class="fa-solid fa-trash"></i></button></div>`; }); });
        }
